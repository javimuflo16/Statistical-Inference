...
$$
\mathrm{L}(\mu, \sigma^2; x_1, \dots, x_n) = f(x_1, \dots , x_n; \mu, \sigma^2) = \prod_{i=1}^nf(x_i;\mu,\sigma^2)= \frac{1}{(\sigma\sqrt{2\pi})^n\prod_{i=1}^nx_i}\exp\left(-\frac{1}{2\sigma^2}\sum_{i=1}^n(log{(x_i)-\mu})^2\right)
$$
...
& = \log\left[\frac{1}{(\sigma\sqrt{2\pi})^n\prod_{i=1}^nx_i}\exp\left(-\frac{1}{2\sigma^2}\sum_{i=1}^n(log{(x_i)-\mu})^2\right)\right] \\ 
& = - \log(\sigma\sqrt{2\pi})^n-\sum_{i=1}^n \log(x_i)-\frac{1}{2\sigma^2}\sum_{i=1}^n(\log{x_i}-\mu)^2
\end{align*}
...
$$
\frac{\partial}{\partial{\mu}}\frac{-1}{2\sigma^2}\sum_{i=1}^n(\log{x_i}-\mu)^2 =  \frac{-1}{2\sigma^2}\sum_{i=1}^n\frac{\partial}{\partial{\mu}}(\log{x_i}-\mu)^2 = \frac{-1}{2\sigma^2}\sum_{i=1}^n-2(\log{x_i}-\mu) = \frac{1}{\sigma^2}\sum_{i=1}^n(\log{x_i}-\mu)
$$

and with respect $\sigma^2$ we have that

$$
\frac{\partial}{\partial{\sigma^2}}-\log(\sqrt{2\pi\sigma^2})^n-\frac{1}{2\sigma^2}\sum_{i=1}^n(\log{x_i}-\mu)^2 = -\frac{n}{2\sigma^2}+\frac{1}{2\sigma^4}\sum_{i=1}^n(\log{x_i}-\mu)^2
$$. 

The parameters are calculated by equaling both equations to zero, then we obtain that $\hat\mu_{MLE}=\frac{1}{n}\sum_{i=1}^n\log{x_i}$ and $\hat\sigma^2_{MLE}=\frac{1}{n}\sum_{i=1}^n(\log{x_i}-\hat\mu)^2$. In this case, the maximum likelihood estimators are identical to those for a Normal distribution where the observations have applied the the log transformation.

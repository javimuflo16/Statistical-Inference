...
$$ CV_{\hat{\mu}_{1}} = \frac{\sqrt{Var(\hat{\mu}_{1})}}{E[\hat{\mu}_{1}]} = \frac{\sqrt{Var(X)}}{\sqrt{n}E[X]} = \frac{\sqrt{(e^{\sigma^2}-1)e^{2\mu + \sigma^2}}}{\sqrt{n}e^{\mu + \frac{\sigma^2}{2}}} = \frac{\sqrt{(e^{\sigma^2}-1)e^{(\mu + \frac{\sigma^2}{2})^2}}}{\sqrt{n}e^{\mu + \frac{\sigma^2}{2}}} = \frac{\sqrt{(e^{\sigma^2}-1)}e^{(\mu + \frac{\sigma^2}{2})}}{\sqrt{n}e^{\mu + \frac{\sigma^2}{2}}} = \frac{\sqrt{(e^{\sigma^2}-1)}}{\sqrt{n}}$$
...
$$ \frac{\sqrt{(e^{\hat{\sigma}^2_{mm}}-1)}}{\sqrt{n}}$$
...
cv <- cbind(cv_1, cv_2)
colnames(estimators) <- c("CV 1", "CV 2")
rownames(estimators) <- c("Error of estimations")

knitr::kable(estimators, caption="Error of estimations") %>%
column_spec(1, bold=TRUE) %>% row_spec(0, bold=TRUE) %>%
kable_styling(latex_options = "HOLD_position", font_size = 7)
```

...
We would like to extrapolate our results (top 200 trending videos per day) to the whole population of YouTube videos. At the beginning just for the whole YouTube and later on segmented by each category.
...
```{r create_trans_form, cache=TRUE, eval=FALSE, include = FALSE}
# 2.
...
& = \log{\left[\frac{1}{\left(\sqrt{2\pi\sigma^2}\right)^n\prod_{i=1}^nx_i}\exp\left(-\frac{1}{2\sigma^2}\sum_{i=1}^n(log{(x_i)-\mu})^2\right)\right]} = -\log\left(\sqrt{2\pi\sigma^2}\right)^n-\sum_{i=1}^n \log(x_i)-\frac{1}{2\sigma^2}\sum_{i=1}^n(\log{x_i}-\mu)^2 \\
& = -\frac{n}{2}\log(2\pi\sigma^2)-\sum_{i=1}^n\log(x_i)-\frac{1}{2\sigma^2}\sum_{i=1}^n((\log{x_i})^2-2\mu\log{x_i}+\mu^2) = -\frac{n}{2}\log(2\pi\sigma^2)-\sum_{i=1}^n\log(x_i)-\frac{\sum_{i=1}^n(\log{x_i})^2}{2\sigma^2}+\frac{\sum_{i=1}^n\mu\log{x_i}}{\sigma^2}-\frac{n\mu^2}{2\sigma^2}
\end{align*}
\endgroup
...
& = \frac{\partial}{\partial{\mu}}\frac{\sum_{i=1}^n\mu\log{x_i}}{\sigma^2}-\frac{n\hat\mu^2}{2\sigma^2} = \frac{\sum_{i=1}^n\log{x_i}}{\sigma^2}-\frac{n\hat\mu}{\sigma^2} \rightarrow \frac{\sum_{i=1}^n\log{x_i}}{\sigma^2}-\frac{n\hat\mu}{\sigma^2}= 0 \rightarrow n\hat\mu = \sum_{i=1}^n\log{x_i} \\
...
We use the estimations of the model parameters carried out using the method of moments for calculating the final value of the CV, i.e. $\hat{\mu}_{mm}$ and $\hat{\sigma}^2_{mm}$. We use these specified values because they are more accurate than the ones obtained with maximum likelihood method. Thus, $CV_{\hat{\mu}_{1}} = \frac{\sqrt{(e^{\hat{\sigma}^2_{mm}}-1)}}{\sqrt{n}}\times 100 = 1.86\%$. It is also important to mention that this estimation of the variance will be used for the following sections, since it will be necessary to compute the calculations required.
...
We approximate $\bar{X}$ of `views` by the Central Limit Theorem $\frac{\bar{X}_n-\mu}{\frac{\sigma}{\sqrt{n}}} \sim \mathcal{N}(0,1)$ for a large $n$. Hence, the 95% confidence interval for the population mean can be computed as follows $\mathrm{CI}_{0.95}(\mu) = \bar{X} \pm Z_{\alpha/2}\frac{\sigma}{\sqrt{n}}$. Thus:

$$
\small
\mathrm{CI}_{0.95}(\mu) = (\bar{X} - Z_{0.025}\frac{\hat{\sigma}_{mm}}{\sqrt{n}}, \bar{X} + Z_{0.025}\frac{\hat{\sigma}_{mm}}{\sqrt{n}})
$$

The confidence interval (upper and lower bounds) obtained for $\mu$ is $(561.1267, 561.1449)$.
...
CI_lower <- mu_1 - qnorm(.975)*(sigma_mm/sqrt(n))
CI_upper <- mu_1 + qnorm(.975)*(sigma_mm/sqrt(n))
...
Here we need to calculate the variance of the estimator $\mathbb{V}\mathrm{ar}[\hat{\pi_4}] = \mathbb{V}\mathrm{ar}\left[\frac{1}{n}\sum_{i=1}^{n}X_i\right] = \frac{1}{n^{2}}\sum_{i=1}^{n}\mathbb{V}\mathrm{ar}[X_i] = \frac{1}{n}\mathbb{V}\mathrm{ar}[X]$. Thus, since we know that $\mathbb{V}\mathrm{ar}[X]$ is equal to $\pi_k(1-\pi_k)$ and that the categorical distribution is a generalized form of the Bernoulli distribution, we can estimate the variance of $\hat{\pi_4}$ as follows $\hat{\sigma}_{\hat{\pi_4}} = \frac{1}{n}\hat{\pi_4}(1-\hat{\pi_4}) = 4 \times 10^{-07}$. This value is considerable smaller, being a good quality of an estimator, since the variance depends on the sample size, $\frac{1}{n}$, and this value in our data set is sizeable.
...
$$
\small
\mathrm{CI}_{0.95}(\pi) = (\hat{\pi} - Z_{0.025}\sqrt{\frac{\hat{\pi}(1-\hat{\pi})}{n_{music}}}, \hat{\pi} + Z_{0.025}\sqrt{\frac{\hat{\pi}(1-\hat{\pi})}{n_{music}}}) = (0.0869, 0.0954)
$$
...
CI_p_lower <- prop_music - qnorm(.975)*(sqrt(prop_music*(1-prop_music)/n_music))
CI_p_upper <- prop_music + qnorm(.975)*(sqrt(prop_music*(1-prop_music)/n_music))
...
$$
CI_{0.95}(\mu_{film}-\mu_{music}) =
$$
$$
=((\hat{\mu}_{film}-\hat{\mu}_{music}) - Z_{0.025}\sqrt{\frac{\hat{\sigma^{2}}_{film}}{n_{film}} + \frac{\hat{\sigma^{2}}_{music}}{n_{music}}}, (\hat{\mu}_{film}-\hat{\mu}_{music}) + Z_{0.025}\sqrt{\frac{\hat{\sigma^{2}}_{film}}{n_{film}} + \frac{\hat{\sigma^{2}}_{music}}{n_{music}}}  
$$
...
CI_submean_lower <- (mu_film - mu_music) - qnorm(.975)*
  (sqrt((s2_mm_film / n_film) + (s2_mm_music / n_music)))
mean_diff <- mu_film - mu_music
CI_submean_upper <- (mu_film - mu_music) + qnorm(.975)*
  (sqrt((s2_mm_film / n_film) + (s2_mm_music / n_music)))
...
$$
\small
Z = \frac{\hat{\mu}_{film}-\hat{\mu}_{music}}{\sqrt{\frac{\hat{\sigma^{2}}_{film}}{n_{film}} + \frac{\hat{\sigma^{2}}_{music}}{n_{music}}}} \approx N(0,1)
$$
Thus, we reject $H_0$ if $|Z| > Z_{\alpha/2}$, i.e. if $|Z| > Z_{0.025}$. The results is $|Z|=95658.03$ related to a $|Z_{0.025}|=1.96$. Hence, since the value of the test statistic is higher than $Z_{0.025}$ we reject the null hypothesis and we can confirm that both means are not equal at all.

```{r hypothesis_submean_equal, echo = FALSE}

# Hypothesis testing to compare means of two subgroups

Z <- (mu_film-mu_music)/sqrt((s2_mm_film/n_film) + (s2_mm_music/n_music))
Z_alpha2 <- qnorm(0.025)
```
...
$$
\small
\mathbb{P}\left[-Z_{\alpha/2} \leq \frac{\hat\pi_{film}-\hat\pi_{music}-(\pi_{film}-\pi_{music})}{\sqrt{\sigma^2_{film-music}}} \leq Z_{\alpha/2} \right] = 0.95 \rightarrow \\
$$
$$
\small
\rightarrow \mathbb{P}\left[(\hat\pi_{film}-\hat\pi_{music})-Z_{\alpha/2}\sqrt{\sigma^2_{film-music}} \leq \pi_{film}-\pi_{music} \leq
(\hat\pi_{film}-\hat\pi_{music})+Z_{\alpha/2}\sqrt{\sigma^2_{film-music}} \right] = 0.95
$$

$$
\small
\mathrm{CI}(\pi_{film}-\pi_{music}) := (\hat\pi_{film}-\hat\pi_{music})\pm Z_{0.025}\sqrt{\frac{\hat\pi_{film}(1-\hat\pi_{film})}{n_{film}}+\frac{\hat\pi_{music}(1-\hat\pi_{music})}{n_{music}}}
$$
...
colnames(test_diff) <- c('z-score', 'Z_{0.025}')
...
The next point is about doing one-sample inference, at the beginning we applied some inference methods to a quantitative variable and at the end we worked with a qualitative one. First, we started defining two different estimators of the population mean for the variable `views`: the sample mean and the mean of the highest and the lowest. We checked that the value of the last estimator is considerably larger than the first one since our data set a larger number of observations. Also, we assessed the statistical bias, showing that both estimators are unbiased since their expected value is the same than the expected value of our random variable `views`. In contrast, it does not happen the same for the variance, being much higher for the estimator of the mean of the highest and the lowest. Besides that, we have checked that the only estimator consistent in squared mean is the estimator of the sample mean. The coefficient of variation, since both estimators are unbiased, shows also that the estimator of the mean of the highest and the lowest contains a significantly larger error of the expected value than the other estimator. Hence, we concluded that the estimator of the sample mean is more suitable. We proceeded at this point to calculate a confidence interval for the true value of the population mean, obtaining that it is within 561.128 and 561.144. Notice that the value obtained with the sample mean estimator, 561.135, is included in this interval. The last part of this section, as we mentioned before, we selected a qualitative variable, `category_id`, and we estimate the population proportion of the factor `music` using the sample proportion being 0.0912 its value. This estimator is unbiased as it occurred in the continuous case, and its variance is considerably small since it decreases when the sample size grows. We have calculated a confidence interval for the population proportion which is within the interval 0.090 and 0.092, again we have checked that the estimated value is within this interval.

In point [4](#inference-with-more-than-one-sample), we applied the same estimation of the population mean for the variable `views` but in this case segmented by the factors of the `category_id` variable. Then, we obtained four different values, having the greatest estimated mean of the views for the category `music`. Therefore, this sub-sample, although its category is not presented in so many other videos, it has the highest mean of visualizations. Again, since we used the sample mean as the estimator, the estimation is unbiased and the coefficient of variation is low (around 2.30% except that the one related to the factor `music` that is slightly higher, a 4.58%). We evaluated the mean squared error of the proportions of each of the factor of the qualitative variable selected before. Then, we obtained that the values associated to this error, for all the factors, are low (around $3 \times 10^{-6}$). Finally, we assessed the confidence interval for the difference of means of the population and also of the proportion with respect two categories: `film` versus `music`. In case of the difference of the means we have obtained an interval of $(-1518.663, -1518.601)$, having the sample mean difference a value of $-1518.632$, negative since we commented before, music has a mean of visualizations considerably much larger. For the other case, the population proportion, we got that the true value is included in $(0.3090, 0.3199)$ and the sample difference of proportions is $0.3144$. We also tested some hypothesis, wanted to demonstrate that the means and the proportions with respect `Film` and `Music` are different for each characteristic. In effect, both test led to reject the null hypothesis of having the same population mean and proportion in the two different sub-samples. Then, it is pretty clear that although the proportion of videos of type `Music` is smaller, the visualizations for this kind of videos are much higher.

## Limitations and extensions

In the future, we could evolve this document filling some gaps we have not covered. For instance, it would be great if we could analyzed other quantitative variables from our data set in order to see if the conclusions we got from the `views` variable are extrapolated to the rest in terms of distribution. Nevertheless, as we saw in Table \ref{tab:table_stats}, the measures of centrality are dispersed among variables, having higher values the variable `likes` than the number of views (in miles), dislikes or comments. 

As we saw before, we drew some conclusions about the number of views per category, in the next iteration it would be great to widen this segmentation to `views` per `category_id` and `continen_id`. Hence, we could get some insights with respect the group of countries where the observation comes from, figuring out if the results we obtained in point [4.1](estimate-the-population-mean-and-the-cv-of-subgroups) are extended to the different continents where the videos are visualized.

The limitations we have identified reside in the number of countries we have in our data set that, as we mentioned at the beginning of this document, are just 10: Canada, US, Germany, FR, UK, India, South Korea, Mexico and Russia. Nevertheless, since we have so many observations (195,582 rows) we can say that data are a good picture of the world state at that time, in 2018. As an extension, we could incorporate more data from other different countries to see if there is any variation in comparison with the acquired results. 

...
In this section, we use the continuous variable `views` in order to calculate two different estimators of the population mean. The estimators selected are the following ones:
...
estimators <- c(mu_1, mu_2)
names_estimators <- c("Estimator 1", "Estimator 2")
values_mu <- cbind(names_estimators, estimators)
knitr::kable(values_mu, col.names = c("Estimator", "Value of estimations"))
```

Now, let's analyze the properties of both estimators. First, we will see if any of them is unbiased.

- $$ E[\hat{\mu}_{1}] = E[\bar{X}] = E[\frac{1}{n} \sum_{i=1}^{n} X_{i}] = \frac{1}{n} \sum_{i=1}^{n} E[X_i] = E[X] $$

- $$ E[\hat{\mu}_{2}] = E[\frac{X_{1} + X_{n}}{2}] = \frac{1}{2}E[X_i + X_n] = E[X] $$

Hence both estimators are unbiased. Now, let's calculate the the variance of each one.

- $$ V(\hat{\mu}_{1}) = V(\bar{X}) = V(\frac{1}{n} \sum_{i=1}^{n} X_{i}) = \frac{1}{n^2} \sum_{i=1}^{n} V(X_i) =  \frac{V(X)}{n} $$

- $$ V(\hat{\mu}_{2}) = V(\frac{X_{1} + X_{n}}{2}) = \frac{1}{4}V(X_i + X_n) =  \frac{1}{2}V(X)$$
...
Other property we can analyse is the consistency in squared mean. We know that if the bias and the variance tend to zero as $n \to \infty$. Therefore, since both estimators are unbiased but $\hat{\mu}_{2}$ does not tend to zero in the limit, so the only consistent estimator is $\hat{\mu}_{2}$.
...
- $$ CV_{\hat{\mu}_{1}} = \frac{\sqrt{Var(\hat{\mu}_{1})}}{E[\hat{\mu}_{1}]} = \frac{\sqrt{Var(X)}}{\sqrt{n}E[X]} = $$

- $$ CV_{\hat{\mu}_{1}} = \frac{\sqrt{Var(\hat{\mu}_{1})}}{E[\hat{\mu}_{1}]} = \frac{\sqrt{Var(X)}}{\sqrt{2}E[X]} = $$
...
print("The estimated proportion is:", prop_music)
prop_music
```
...
$$
V(\hat{\pi_4}) = V(\frac{1}{n}\sum_{i=1}^{n}X_i) = \frac{1}{n^{2}}\sum_{i=1}^{n}V(X_i) = \frac{1}{n}V(X)
$$
...
$$
\hat{\sigma}_{\hat{\pi_4}} = \frac{1}{n}\hat{\pi_k}(1-\hat{\pi_k})
$$

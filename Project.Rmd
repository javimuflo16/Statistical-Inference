...
Recall that the log-normal distribution has $\mathbb{E}[X]=e^{\mu+\sigma^2/2}$ and the normal distribution for the log transformation has $\mu=\mathbb{E}[\log{X}]$, then we can formulate the parts of our system of equations. In the population we have that $\alpha_{1_\mathcal{N}} = \mu = \mathbb{E}[\log{X}]$ and $\alpha_{1_{\mathcal{LN}}} =\exp\{\hat\mu_{mm}+\hat{\sigma}_{mm}^2/2\} = \mathbb{E}[X]$ and in the sample side we have that $\mathrm{a}_{1_\mathcal{N}} = \frac{1}{n}\sum_{i=1}^n\log{x_i} = \overline{\log{x}}$ and $\mathrm{a}_{1_\mathcal{LN}}=\frac{1}{n}\sum_{i=1}^nx_i=\bar{x}$, then equating $\alpha_{1_\mathcal{N}}=\mathrm{a}_{1_\mathcal{N}}$ and $\alpha_{1_{\mathcal{LN}}} = \mathrm{a}_{1_\mathcal{LN}}$ we get to our system of equations:

$$
    \begin{cases}
      \hat\mu_{mm} = \overline{\log{x}}\\
      e^{\hat\mu_{mm}+\hat{\sigma}_{mm}^2/2} = \bar{x}
    \end{cases}\,
$$
Resolving analytically the second equations we have that:

\begin{align*}
e^{\hat\mu_{mm}}e^{\hat{\sigma}_{mm}^2/2}
      & = e^{\frac{1}{n}\sum_{i=1}^n\log{x_i}}e^{\hat{\sigma}_{mm}^2/2} \\ 
      & = e^{\sum_{i=1}^n\log x^{1/n}}e^{\hat{\sigma}_{mm}^2/2} \\ 
      & = e^{\log x_1^{1/n}}\dots e^{\log x_n^{1/n}} e^{\hat{\sigma}_{mm}^2/2} \\ 
      & = x_1^{1/n}\dots x_n^{1/n}e^{\hat{\sigma}_{mm}^2/2} \\ 
      & = \prod_{i=1}^n x_i^{1/n}e^{\hat{\sigma}_{mm}^2/2} \\ 
      & = \mathrm{geo}(x)e^{\hat{\sigma}_{mm}^2/2}
\end{align*}

Then, $\mathrm{geo}(x)e^{\hat{\sigma}_{mm}^2/2} = \bar{x} \rightarrow \sigma^2_{mm} = 2\log\left(\frac{\bar{x}}{\mathrm{geo}(x)}\right)$
...
mu_mm <- mean(final_data$log_views)
sigma_mm <- 2*log(mean(final_data$views)/geo(final_data$views))
print(mu_mm)
print(sigma_mm)

curve(dlnorm(x, mu_mm, sqrt(sigma_mm)), from=0, to=500, col=4, ylab="frequency", xlab="views")
```

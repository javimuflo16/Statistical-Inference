...
Our random variable $X_{category\_id} \sim \mathrm{Multin}(n,\mathbf{p})$ with parameters $n$ and $\mathbf{p}$, where $n=195582$ and $\mathbf{p}=(p_1, p_2, p_3, p_4)=(0.4056, 0.0912, 0.2200, 0.2832)$. Now, let's calculate the MSE for each factor of the categorical variable. Since the estimator of the proportion is unbiased, recall that an unbiased estimator has  $\mathrm{Bias}(\hat{p_i})=\mathbb{E}[\hat{p_i}]-p_i=0$ and then $\mathbb{E}[\hat{p_i}] = \mathbb{E}[X]=p_i$, therefore $\mathrm{MSE}(\hat{p_i})=\mathrm{Bias}^2(\hat{p_i})+\mathbb{V}\mathrm{ar}[\hat{p_i}]=\mathbb{V}\mathrm{ar}[\hat{p_i}]=\frac{1}{n}\hat{p}_i(1-\hat{p}_i)$, then $\hat{\mathrm{MSE}_i}=\frac{1}{n}\hat{p}_i(1-\hat{p}_i)$. In the Table \ref{tab:mse_cat_tot} is shown these estimations for each category. As we can see, the mean squared errors in all the cases are considerably small, this is because the sample size is really large.
...
mses <- rbind(mse_film, mse_music, mse_education, mse_leisure)
colnames(mses) <- c('MSE_hat')
rownames(mses) <- c('Film', 'Music', 'Education', 'Leisure')
...
## Comparison of the means for the two subgroups
...
CI_submean_lower <- (mu_music - mu_education) - qnorm(.95)*(sqrt((s2_education/n_education) + (s2_music/n_music)))
mean_diff <- mu_music - mu_education
CI_submean_upper <- (mu_music - mu_education) + qnorm(.95)*(sqrt((s2_education/n_education) + (s2_music/n_music)))
...
rownames(CI_submean) <- c("Confidence interval for population mean subgroups")
...
## Compare the means of for the two subgroups by means of hypothesis testing of equality of means
...
Z <- (mu_music-mu_education)/sqrt((s2_music/n_music)+(s2_education/n_education))

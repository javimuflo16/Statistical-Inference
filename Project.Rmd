...
$$
\mathbb{V}\mathrm{ar}(\hat{\mu}_{1}) = \mathbb{V}\mathrm{ar}(\bar{X}) = \mathbb{V}\mathrm{ar}(\frac{1}{n} \sum_{i=1}^{n} X_{i}) = \frac{1}{n^2} \sum_{i=1}^{n} \mathbb{V}\mathrm{ar}(X_i) =  \frac{\mathbb{V}\mathrm{ar}(X)}{n} = \frac{(e^{\sigma^2}-1)}{n}e^{2\mu + \sigma^2}
$$

$$
\mathbb{V}\mathrm{ar}(\hat{\mu}_{2}) = \mathbb{V}\mathrm{ar}(\frac{X_{1} + X_{n}}{2}) = \frac{1}{4}\mathbb{V}\mathrm{ar}(X_i + X_n) =  \frac{1}{2}\mathbb{V}\mathrm{ar}(X) = \frac{(e^{\sigma^2}-1)}{2}e^{2\mu + \sigma^2}
$$
...
$$
CV_{\hat{\mu}_{1}} = \frac{\sqrt{\mathbb{V}\mathrm{ar}(\hat{\mu}_{1})}}{\mathbb{E}[\hat{\mu}_{1}]} = \frac{\sqrt{\mathbb{V}\mathrm{ar}(X)}}{\sqrt{n}\mathbb{E}[X]} = \frac{\sqrt{(e^{\sigma^2}-1)e^{2\mu + \sigma^2}}}{\sqrt{n}e^{\mu + \frac{\sigma^2}{2}}} =
$$
...
$$
\mathrm{CV}_{\hat{\mu}_{2}} = \frac{\sqrt{\mathbb{V}\mathrm{ar}(\hat{\mu}_{2})}}{\mathbb{E}[\hat{\mu}_{2}]} = \frac{\sqrt{\mathbb{V}\mathrm{ar}(X)}}{\sqrt{2}\mathbb{E}[X]} = \frac{\sqrt{(e^{\sigma^2}-1)}}{\sqrt{2}} = \frac{\sqrt{(e^{\hat{\sigma}^2_{mm}}-1)}}{\sqrt{2}}
$$
...
In this case, we are going to select the qualitative variable `category_id` to divide the population of the variable `views` into subgroups. Then, there will be four different groups associated to the factors `Education`, `Film`, `Leisure` and `Music`. Let $\bar{x}_{education}$, $\bar{x}_{film}$, $\bar{x}_{leisure}$ and $\bar{x}_{music}$ be the sample means of the number of views for each segment and let $s^2_{education}$, $s^2_{film}$, $s^2_{leisure}$ and $s^2_{music}$ be the sample variances of the number of views for each subgroup. 

The estimation of the population mean can be obtained from the sample mean of each subgroup. Then, we have that $\hat\mu_{education} = \bar{x}_{education}$, $\hat\mu_{film} = \bar{x}_{film}$, $\hat\mu_{leisure} = \bar{x}_{leisure}$ and $\hat\mu_{music} = \bar{x}_{music}$.

Knowing that the formula of the coefficient of variation of the mean is: $\mathrm{CV} = \frac{\mathrm{SE}}{\bar{x}}\times 100 = \frac{\sqrt{s^2}}{\bar{x}\sqrt{n}} \times 100$ where $\mathrm{SE}$ stands for the standard error of the mean, we have that
$$
\mathrm{CV}_{\mu_{education}} = \frac{\sqrt{s^2}_{education}}{\bar{x}_{education}\sqrt{n}} \times 100 \quad 
\mathrm{CV}_{\mu_{film}} = \frac{\sqrt{s^2}_{film}}{\bar{x}_{film}\sqrt{n}} \times 100
$$
$$
\mathrm{CV}_{\mu_{leisure}} = \frac{\sqrt{s^2}_{leisure}}{\bar{x}_{leisure}\sqrt{n}} \times 100 \quad 
\mathrm{CV}_{\mu_{music}} = \frac{\sqrt{s^2}_{music}}{\bar{x}_{music}\sqrt{n}} \times 100 
$$
...
ns <- c(n_education, n_film, n_leisure, n_music)
...
s2s <- c(s2_education, s2_film, s2_leisure, s2_music)

n <- length(final_data$views)

cv_education <- sqrt(s2_education)/(sqrt(n)*mu_education)*100
cv_film <- sqrt(s2_film)/(sqrt(n)*mu_film)*100
cv_leisure <- sqrt(s2_leisure)/(sqrt(n)*mu_leisure)*100
cv_music <- sqrt(s2_music)/(sqrt(n)*mu_music)*100

cvs <- c(cv_education, cv_film, cv_leisure, cv_music)

data_groups <- cbind(ns, mus, s2s, cvs)
colnames(data_groups) <- c('size', 'mu', 'sigma^2', 'CV(mu)%')
rownames(data_groups) <- c('Education', 'Film', 'Leisure', 'Music')
...
After proportions have been defined, we are ready to calculate the MSE for each estimator. Since this estimator is unbiased, that holds $\mathrm{Bias}(\hat{\pi_i})=\mathbb{E}[\hat{\pi_i}]-\pi_i=0$ and then $\mathbb{E}[\hat{\pi_i}] = \pi_i = \mathbb{E}[X]$, therefore $\mathrm{MSE}(\hat{\pi_i})=\mathrm{Bias}^2(\hat{\pi_i})+\mathbb{V}\mathrm{ar}[\hat{\pi_i}]=\mathbb{V}\mathrm{ar}[\hat{\pi_i}]=\frac{1}{n}\hat{\pi}_i(1-\hat{\pi}_i)$, then $\hat{\mathrm{MSE}_i}=\frac{1}{n}\hat{p}_i(1-\hat{p}_i)$. In the Table \ref{tab:mse_cat_tot} is shown these estimations for each category. As we can see, the mean squared errors in all the cases are considerably small, this is because the sample size is really large.

```{r mse_cat_tot, echo=FALSE}
mse_education <- 1/n*prop_education*(1-prop_education)
mse_film <- 1/n*prop_film*(1-prop_film)
mse_leisure <- 1/n*prop_leisure*(1-prop_leisure)
mse_music <- 1/n*prop_music*(1-prop_music)

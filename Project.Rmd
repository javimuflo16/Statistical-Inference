...
We approximate $\bar{X}$ of `views` by the Central Limit Theorem $\frac{\bar{X}_n-\mu}{\frac{\sigma}{\sqrt{n}}} \sim \mathcal{N}(0,1)$ for a large $n$. Hence, the 95% confidence interval for the population mean can be computed as follows $\mathrm{CI}_{0.95}(\mu) = \bar{X} \pm Z_{\frac{\alpha}{2}}\frac{\sigma}{\sqrt{n}}$. Thus:

$$
\small
\mathrm{CI}_{0.95}(\mu) = (\bar{X} - Z_{0.975}\frac{\hat{\sigma}_{mm}}{\sqrt{n}}, \bar{X} + Z_{0.975}\frac{\hat{\sigma}_{mm}}{\sqrt{n}})
$$
...
Thus, since we know that $\mathbb{V}\mathrm{ar}(X)$ is equal to $\pi_k(1-\pi_k)$ and that the categorical distribution is a generalized form of the Bernoulli distribution, we can estimate the variance of $\hat{\pi_4}$ as follows $\hat{\sigma}_{\hat{\pi_4}} = \frac{1}{n}\hat{\pi_4}(1-\hat{\pi_4}) = 4 \times 10^{-07}$. This value is considerable smaller, being a good quality of an estimator, since the variance depends on the sample size, $\frac{1}{n}$, and this value in our data set is sizeable.
...
$$
\small
\mathrm{CI}_{0.95}(\pi) = (\hat{\pi} - Z_{0.975}\sqrt{\frac{\hat{\pi}(1-\hat{\pi})}{n_{music}}}, \hat{\pi} + Z_{0.975}\sqrt{\frac{\hat{\pi}(1-\hat{\pi})}{n_{music}}}) = (0.0869, 0.0954)
$$
...
$$
$$
=((\hat{\mu}_{film}-\hat{\mu}_{music}) - Z_{0.975}\sqrt{\frac{\hat{\sigma^{2}}_{film}}{n_{film}} + \frac{\hat{\sigma^{2}}_{music}}{n_{music}}}, (\hat{\mu}_{film}-\hat{\mu}_{music}) + Z_{0.975}\sqrt{\frac{\hat{\sigma^{2}}_{film}}{n_{film}} + \frac{\hat{\sigma^{2}}_{music}}{n_{music}}}  
$$

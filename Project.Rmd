...
Since we know that $X_{views} \sim \mathcal{LN}(\mu, \frac{\sigma}{n})$, we can approximate it by the Central Limit Theorem $\frac{\bar{X}-\mu}{\frac{\sigma}{\sqrt{n}}} \sim \mathcal{N}(0,1)$. Hence, the 95% confidence interval for the population mean can be computed as follows:
...
$$
\mathrm{CV}_{\mu_{education}} = \frac{\sqrt{s^2}_{education}}{\bar{x}_{education}\sqrt{n}} \quad 
\mathrm{CV}_{\mu_{film}} = \frac{\sqrt{s^2}_{film}}{\bar{x}_{film}\sqrt{n}} \quad 
$$
$$
\mathrm{CV}_{\mu_{leisure}} = \frac{\sqrt{s^2}_{leisure}}{\bar{x}_{leisure}\sqrt{n}}
\mathrm{CV}_{\mu_{music}} = \frac{\sqrt{s^2}_{music}}{\bar{x}_{music}\sqrt{n}} \quad 
$$
...
cv_education <- sqrt(s2_education)/(sqrt(n)*mu_education)
cv_film <- sqrt(s2_film)/(sqrt(n)*mu_film)
cv_leisure <- sqrt(s2_leisure)/(sqrt(n)*mu_leisure)
cv_music <- sqrt(s2_music)/(sqrt(n)*mu_music)
